...
from networks.model import CSGNN
...
def train_class(model, loaders, epochs=200, patience=100, lr=1e-2, r=None, print_errors=False):
    ...
    early_stopper = Patience(patience=patience, use_loss=False,
                             save_path=os.path.join("models/", args.dataset, "model" + now + ".pth.tar"))
    ...
    for epoch in pbar_train:
        ...
        pbar_train.set_description(
            f"Epoch {epoch + 1} Train Loss {train_loss:0.3f} Train Acc {train_acc:0.3f} Val loss {round(val_loss, 3)} Val acc {round(val_acc, 3)} Best Val Loss {round(early_stopper.val_loss, 3)} Best Val Acc {early_stopper.val_acc:0.3f}")
    ...
    log.print_and_log(
        f"Train Acc {(train_correct / train_count):0.2f} Val Acc {(val_correct / val_count):0.2f} Test Acc {(test_correct / test_count):0.2f}")
...
def main_class(dataset, model_config, train_config):
    ...
    model = CSGNN(device=device, **model_config).to(device)
...
def main_iso(dataset, model_config, train_config):
    loader = DataLoader(dataset,
                        batch_size=train_config["batch_size"],
                        shuffle=False)
    model = CSGNN(predict=False, device=device, **model_config).to(device)
    res = []
    for i in range(5):
        torch.manual_seed(i)
        model.reset_parameters()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default="EXP",
                        help='Experimental dataset name (default: EXP)')
    ...
    logging.basicConfig(filename=os.path.join('logs', args.dataset, args.dataset + now + '.log'), level=logging.INFO,
                        filemode="w")
    ...
    #path_types = [args.path_type] if not args.path_type == "all" else ["shortest_path", "all_shortest_paths",
    #                                                                   "all_simple_paths"]
    ...
    for d_name in DATASET_NAMES[args.dataset]:
        ...
        if args.dataset == 'SR25':
            config['cycles']["dataset_config"]['dataset_name'] = d_name
        dataset = DATASETS[args.dataset](**config['cycles']["dataset_config"])
        results['cycles'][d_name] = MAIN_FUNC[config["task"]](
            dataset,
            config['cycles']["model_config"],
            config['cycles']["train_config"])

        if config["task"] == "classification":
            log.print_and_log(
                f"Train Avg {results['cycles'][d_name]['train'].mean():0.4f} Train Std {results['cycles'][d_name]['train'].std():0.4f}")
            log.print_and_log(
                f"Val Avg   {results['cycles'][d_name]['val'].mean():0.4f} Val Std   {results['cycles'][d_name]['val'].std():0.4f}")
            log.print_and_log(
                f"Test Avg  {results['cycles'][d_name]['test'].mean():0.4f} Test Std  {results['cycles'][d_name]['test'].std():0.4f}")

        elif config["task"] == "iso":
            log.print_and_log(f"Avg # of similar pairs: {results['cycles'][d_name]['n_sim_pairs'].mean()}")
            log.print_and_log(f"Std # of similar pairs: {results['cycles'][d_name]['n_sim_pairs'].std()}")
